        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List of Socio Psyche Counseling</h4>
                       
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i>Add Socio Psyche Counseling</button>
                    </div>
                </div>
              ...
              <div class="table-responsive mb-0" data-pattern="priority-columns">
                  <table id="tech-companies-1" class="table table-bordered align-middle nowrap ">
                    ...
                    <!-- <table class="table table-bordered dt-responsive  nowrap w-100"> -->
                        ...
                        <tbody>
                        <tr v-for="(workshop, index) of workshops" class="text-center align-middle">
                            <td>{{ index+1 }}</td>
                            <td>{{ workshop.title }}</td>

                            <td>{{moment(workshop.start_date).format('ll')}} <span style="font-weight: bold">-</span> {{moment(workshop.end_date).format('ll')}}</td>
                            <td>
                            <router-link :to="{ name: 'app.workshop_application' , query: { workshopId: workshop.id }}" class="btn btn-sm btn-success">
                              <i class="fa-solid fa-person-circle-check"></i> {{ workshop.workshop_applications }}
                            </router-link>

                            </td>
                            <td>{{ workshop.status }}</td>
                            <td >
                                <span v-if="workshop.current_date <= workshop.end_date" class="badge bg-success"><b>Activated</b></span>
                                <span v-else class="badge bg-danger">DeActivated</span>
                            </td>
                            <td>
                                <button type="button" @click="show(workshop)" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </button>

                                <button type="button" @click="edit(workshop)" class="btn btn-primary btn-sm" >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button type="button" @click="destroy(workshop.id)" class="btn btn-danger btn-sm">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            </td>
                        </tr>

                        <tr v-show="!workshops.length">
                            <td colspan="8">
                    ...
                    <div v-if="workshops.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="workshops.length">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Socio Psyche Counseling</h5>
    ...
    export default {
        ...
        methods: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}workshop`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "Workshop Added" });
                        }else{
                            this.$Progress.fail()
