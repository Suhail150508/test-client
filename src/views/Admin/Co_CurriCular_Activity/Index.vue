        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List Of Co-Curricular activities</h4>
                        <span>
                            This is a Co-Curricular activities related information
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i> Add Co-Curricular activities</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tbody>
                        <tr v-for="(co_curricular, index) of co_curriculars">
                            <td>{{ co_curricular.id }}</td>
                            <td>{{ co_curricular.title }}</td>
                            <td>{{ co_curricular.description }}</td>
                            <td>{{ co_curricular.status }}</td>
                            <td>
                                <button type="button" @click="show(co_curricular)" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </button>

                                <button type="button" @click="edit(co_curricular)" class="btn btn-primary btn-sm" >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button type="button" @click="destroy(co_curricular.id)" class="btn btn-danger btn-sm">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            </td>
                        </tr>

                        <tr v-show="!co_curriculars.length">
                            <td colspan="6">
                    ...
                    <div v-if="co_curriculars.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="co_curriculars.length">
...
<script>
    ...
    export default {
        ...
        methods: {
            getData(url) {
                this.$Progress.start();
                let linkUrl = url ? url :  `${this.backendUrl}co-curricular-activity`;
                axios.get(linkUrl, {
                    params: {
                    ...
                    .then((response) => {
                        this.co_curriculars = response.data.data;
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}co-curricular-activity`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "co_curricular Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}co-curricular-activity/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "co_curricular Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}co-curricular-activity/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
