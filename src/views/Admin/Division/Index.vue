      ...
      <div class="card">
        <div class="row card-header bg-transparent border-bottom">
          <div class="col-md-8">
            ...
            <span>This is a Divisions related information</span>
          </div>
        ...
        <div class="card-body">
          ...
          <table class="table table-bordered dt-responsive nowrap w-100">
            ...
            <tbody>
              <tr v-for="(division, index) of divisions">
                <td>{{ index + 1 }}</td>
                <td>{{ division.name }}</td>
                <!-- <td>{{ division.bn_name }}</td> -->
                <td>Published</td>
                <td>
                  <!-- <button type="button" @click="show(division)" class="btn btn-info btn-sm">
                    <i class="fas fa-eye"></i>
                  </button> -->

                  <button type="button" @click="edit(division)" class="btn btn-primary btn-sm" style="margin-right: 5px;">
                    <i class="fas fa-edit"></i>
                  </button>

                  <button type="button" @click="destroy(division.id)" class="btn btn-danger btn-sm">
                    <i class="fas fa-trash-alt"></i>
                  </button>
                </td>
              </tr>

              <tr v-show="!divisions.length">
                <td colspan="6">
          ...
          <div v-if="divisions.length" class="d-flex">
            <div class="col-md-4 pl-0" style="margin: auto" v-if="divisions.length">
            ...
            <div class="modal-body">
              <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)">
                ...
                <div class="modal-body">
                  ...
                  <!-- <div class="form-group mb-3">
                    <label>bn_name</label>
                    <input v-model="form.bn_name" type="text" name="bn_name" class="form-control" placeholder="bn_name"/>
...
export default {
  ...
  methods: {
    getData(url) {
      this.$Progress.start();
      let linkUrl = url ? url : `${this.backendUrl}division`;
      axios
        .get(linkUrl, {
          params: {
    ...
    store() {
      ...
      this.form.busy = true;
      this.form
        .post(`${this.backendUrl}division`)
        .then((response) => {
          this.getData();
          $("#exampleModal").modal("hide");
          if (this.form.successful) {
            this.$Progress.finish();
            this.$notify({ type: "success", title: "Success", text: "Division Added" });
          } else {
            this.$Progress.fail();
    ...
    update() {
      ...
      this.form.busy = true;
      this.form
        .put(`${this.backendUrl}division/` + this.form.id)
        .then((response) => {
          this.getData();
          $("#exampleModal").modal("hide");
          if (this.form.successful) {
            this.$Progress.finish();
            this.$notify({ type: "success", title: "Success", text: "Division Updated" });
          } else {
            this.$Progress.fail();
            this.$notify({ type: "error", title: "Error", text: "Something went wrong try again later" });
          }
    ...
    destroy(id){
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(`${this.backendUrl}division/`+id, )
          .then(response => {
