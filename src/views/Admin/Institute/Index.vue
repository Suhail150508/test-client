            ...
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        ...
                        <span>
                            This is an Institute related information
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i>Add Institute</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tbody>
                        <tr v-for="(institute, index) of institutes">
                            <td>{{ index+1 }}</td>
                            <td>{{ institute.name }}</td>
                            <td>{{ institute.type }}</td>
                            <td>{{ institute.status }}</td>
                            <td>
                                <button type="button" @click="show(institute)" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </button>

                                <button type="button" @click="edit(institute)" class="btn btn-primary btn-sm" >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button type="button" @click="destroy(institute.id)" class="btn btn-danger btn-sm">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            </td>
                        </tr>

                        <tr v-show="!institutes.length">
                            <td colspan="6">
                    ...
                    <div v-if="institutes.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="institutes.length">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Institute</h5>
...
<script>
    ...
    export default {
        ...
        methods: {
            getData(url) {
                this.$Progress.start();
                let linkUrl = url ? url :  `${this.backendUrl}institute`;
                axios.get(linkUrl, {
                    params: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}institute`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "Institute Created Successfully" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}institute/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "Institute Updated Successfully" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}institute/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
