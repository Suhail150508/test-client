      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="row mb-3">
            <div class="col-md-3">
              <div class="row">
                ...
                <div class="col-md-3">
                  <select @change="getData(null)" v-model="perPage" class="form-control form-control-sm">
            ...
            <div class="col-md-3">
              <div class="dropdown ml-4">
                ...
                <div class="dropdown-menu">
                  ...
                  <a href="#" class="dropdown-item" type="button" @click.prevent="downloadResume">
                    Download resume
                  </a>
                </div>
              </div>
            ...
            <div class="col-md-6">
              <div class="row">
                ...
                <div class="col-md-3">
                  <select v-model="fieldName" id="fields" class="form-control form-control-sm">
                    <option value="first_name">First Name</option>
                    <option value="middle_name">Middle Name</option>
                    <option value="last_name">Last Name</option>
                    <option value="email">Email</option>
                    <option value="gender">Gender</option>
                    <option value="ewu_id_no">ewu_id_no</option>
                    <option value="blood_group">Blood_group</option>
                    <option value="present_salary">present_salary</option>
                    <option value="expected_salary">expected_salary</option>
                    <option value="board">Board</option>
                    <option value="degree">Degree</option>
                    <option value="grade">Grade</option>
                    <option value="passing_year">Passing Year</option>
                    <option value="major_id">Major</option>
                    <option value="minor_id">Minor</option>
              ...
              <table id="tech-companies-1" class="table table-bordered align-middle nowrap ">
                <thead>
                  <tr class="table-secondary">
                    ...
                    <th scope="col">Blud Group</th>
                    <th scope="col">Availble For</th>
                    <th scope="col"> Email</th>
                    <th scope="col"> EWU ID NO</th>
                ...
                <tbody>
                  <tr v-for="item in applicants">
                    <td class="text-center"><input type="checkbox" :value="item.id" v-model="checked"/></td>
                    <th >
                      <img v-if="item.profile_image" :src="item.profile_image" class="rounded" style="width:60px;height: 65px;" width="250" :alt="Image">
                      <img v-else :src="$store.state.avatar" class="rounded" style="width:60px;height: 65px;" width="250" :alt="Image">
                    </th>
                    <th >{{ item.first_name }} {{ item.middle_name }} {{ item.last_name }}</th>
                    <th >{{ item.gender }}</th>
                    <th >{{ item.blood_group }}</th>
                    <th v-if="item.career_application">{{ item.career_application.available_for}}</th>
                    <th >{{ item.email }}</th>
          ...
          <div v-if="applicants.length" class="d-flex">
            <div class="col-md-4 pl-0" style="margin: auto" v-if="applicants.length">
            ...
            <div class="modal-body">
              <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)">
                ...
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    <label>Time</label>
                    <input v-model="form.time" type="time" name="time" class="form-control" placeholder="time" :readonly="showMode == true ? true : false"/>
                    <HasError :form="form" field="time" />
                  </div>

                  <div class="form-group">
                    <label>Date</label>
                    <input v-model="form.date" type="date" name="date" class="form-control" placeholder="date" :readonly="showMode == true ? true : false"/>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form @submit.prevent="ratingStore()">
              ...
              <div class="pl-15">
                  <img v-if="this.resume_image" :src="this.resume_image" class="rounded" style="width:150px;height: 160px;" width="250" :alt="Image">
                  <img v-else :src="$store.state.avatar" class="rounded" style="width:150px;height: 160px;" width="250" :alt="Image">
              </div>
              ...
              <div class="modal-body">
                ...
                <div class="row" v-for="(row, index) in ratingForm.ratingFormDetails">                                                 
                  <div class="col-lg-5">
                    ...
                    <Multiselect  :name="'rating['+index+'][skill_ids]'" v-model="row.skill_ids" :searchable="true"  :options="skillData"  placeholder="Select Area of expertice"/>                             
                ...
                <div class="float-right mt-3">
                  <button type="button" class="btn btn-outline-success float-end " @click.prevent="addMoreRatingForm">+Add more</button><br>
                </div>
  ...
  export default {
    ...
    watch: {
      ...
      selectPage: function(value) {
        ...
        if(value){
          this.applicants.forEach(applicant => {
              this.checked.push(applicant.id);
          });
        }else{
          this.checked = [];
      ...
      checked: function(value) {
        this.url = `${this.backendUrl}applicants/export/` + this.checked;
      }
    },
    ...
    methods: {
      ...
      selectAllRecords(){
        axios.get(`${this.backendUrl}alumnis/all`)
        .then(response => {
      ...
      getDependentData() {
        axios.get(`${this.backendUrl}skill`,{ params:{ job_resume_skill: "job_resume_skill" }})
          .then((response) => {
      ...
      store() {
        this.$Progress.start();
        this.form.busy = true;
        this.form.post(`${this.backendUrl}job-post`)
        .then((response) => {
          this.getData();
          $("#exampleModal").modal("hide");
          if (this.form.successful) {
            this.$Progress.finish();
            this.$notify({ type: "success", title: "Success", text: "applicants Added"});
          } else {
            this.$Progress.fail();
      ...
      update() {
        // this.$Progress.start();
        // this.form.busy = true;
        this.form.post(`${this.backendUrl}job-post/` + this.form.id)
        .then((response) => {
          this.getData();
          if (this.form.successful) {
            this.$Progress.finish();
            this.$notify({ type: "success", title: "Success", text: "applicants Updated"});
          } else {
            this.$Progress.fail();
            this.$notify({ type: "error", title: "Error", text: "Something went wrong try again later"});
          }
      ...
      ratingStore() {
        ...
        this.ratingForm.post(`${this.backendUrl}job-profile/store/user-ratings`,{
          params: {
        ...
        .then((response) => {
          ...
          if (this.ratingForm.successful) {
            this.getData();
            this.$Progress.finish();
            this.$notify({type: "success", title: "Success", text: "Data Stored Successfully"});
          } else {
            this.$Progress.fail();
            this.$notify({ type: "error", title: "Error", text: "Something went wrong try again later"});
          }
      ...
      ratingUpdate() {
        this.$Progress.start();
        this.ratingForm.busy = true;
        this.ratingForm.put(`${this.backendUrl}job-profile/update/user-ratings/` + this.ratingForm.id)
        .then((response) => {
          this.getData();
          ...
          if (this.ratingForm.successful) {
            $("#addRating").modal("hide");
            this.$Progress.finish();
            this.$notify({ type: "success", title: "Success", text: "Data  Updated Successfully" });
          } else {
            this.$Progress.fail();
            this.$notify({ type: "error", title: "Error", text: "Something went wrong try again later" });
          }
      ...
      // downloadResume() {
      //   window.location.href = `${this.backendUrl}download/resume/file/${this.checked}`;
      // }

      // downloadResume() {
      //   axios.get(`${this.backendUrl}download/resume/file/${this.checked}`)
      //   .then(response => {
      ...
      //   })
      //   .catch(error => {
      //     this.$notify({ type: "error", title: "Error", text: "An error occurred while downloading the resume" });
      //     console.error(error);
      //   });
      // }
      ...
      downloadResume() {
        // this.checked.forEach(resumeId => {
        //   const url = `${this.backendUrl}download/resume/file/${resumeId}`;
        //   const link = document.createElement('a');
        //   link.href = url;
