        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List of Internship</h4>
                       
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i>Add Internship</button>
                    </div>
                </div>
                  ...
                  <table id="tech-companies-1" class="table table-bordered align-middle nowrap ">
                    ...
                    <!-- <table class="table table-bordered dt-responsive  nowrap w-100"> -->
                        ...
                        <tr class="text-center align-middle">
                            ...
                            <th style="width: 15%">Date (Duration)</th>
                            <th style="width: 5%">Applications <i class="fa-sharp fa-regular fa-file"></i></th>
                            <th style="width: 10%">Status</th>
                            <th style="width: 10%">Validity Status</th>
                        ...
                        <tbody>
                        <tr v-for="(internship, index) of internships" class="text-center align-middle">
                            <td>{{ index+1 }}</td>
                            <td>{{ internship.title }}</td>

                            <td>{{moment(internship.start_date).format('ll')}} <span style="font-weight: bold">-</span> {{moment(internship.end_date).format('ll')}}</td>
                            <td>
                            <router-link :to="{ name: 'app.workshop_application' , query: { internshipId: internship.id }}" class="btn btn-sm btn-success">
                              <i class="fa-solid fa-person-circle-check"></i> {{ internship.internship_applications }}
                            </router-link>

                            </td>
                            <td>{{ internship.status }}</td>
                            <td>
                                <span v-if="internship.current_date  <= internship.end_date" class="badge bg-success"><b>Activated</b></span>
                                <span v-else class="badge bg-danger">DeActivated</span>
                            </td>
                            <td>
                                <button type="button" @click="show(internship)" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </button>

                                <button type="button" @click="edit(internship)" class="btn btn-primary btn-sm" >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button type="button" @click="destroy(internship.id)" class="btn btn-danger btn-sm">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            </td>
                        </tr>

                        <tr v-show="!internships.length">
                            <td colspan="8">
                    ...
                    <div v-if="internships.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="internships.length">
    ...
    export default {
        ...
        methods: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}internships`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "internships Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}internships/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "internships Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}internships/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
