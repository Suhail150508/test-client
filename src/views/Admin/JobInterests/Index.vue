        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List Of Job-Interests</h4>
                        <span>
                            This is an Interest related information about Alumni and Student
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i>Add Interest</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tbody>
                        <tr v-for="(interest, index) of interests">
                            <td>{{ interest.id }}</td>
                            <td>{{ interest.title }}</td>
                            <td>{{ interest.description }}</td>
                            <td>{{ interest.status }}</td>
                            <td>
                                <button type="button" @click="show(interest)" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </button>

                                <button type="button" @click="edit(interest)" class="btn btn-primary btn-sm" >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button type="button" @click="destroy(interest.id)" class="btn btn-danger btn-sm">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            </td>
                        </tr>

                        <tr v-show="!interests.length">
                            <td colspan="6">
                    ...
                    <div v-if="interests.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="interests.length">
...
<script>
    ...
    export default {
        ...
        methods: {
            getData(url) {
                this.$Progress.start();
                let linkUrl = url ? url :  `${this.backendUrl}interest`;
                axios.get(linkUrl, {
                    params: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}interest`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "Interest Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}interest/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "Interest Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}interest/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
