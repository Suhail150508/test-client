    ...
    <div class="col-12">
      <div class="card">
        <div class="row card-header bg-transparent border-bottom">
          ...
          <div class="col-md-4">
            <router-link :to="{ name: 'dashboard_createjob' }" class="btn btn-lg btn-success float-end">
        ...
        <div class="card-body">
              ...
              <div class="row">
                ...
                <div class="col-md-3">
                  <select  v-model="fieldName" id="fields" class="form-control form-control-sm">
                    ...
                    <option value="job_deadline">Job Deadline</option>
              ...
              <table id="tech-companies-1" class="table table-bordered align-middle nowrap ">
                <thead>
                  <tr class="table-secondary">
                    ...
                    <th scope="col"> Applicatin Deadline </th>
                ...
                <tbody>
                  <tr v-for="(item,index) in jobposts">
                    <th>{{ index + 1 }}</th>
                    <th>{{ item.headline }}</th>
                    <th>{{ item.job_title }}</th>
                    <th>{{ item.company_name }}</th>
                    <td>{{moment(item.application_deadline).format("DD-MM-YYYY")}}</td>
                    <template v-if="item.job_details">
                      <td v-html="item.job_details.substring(0, 50) + (item.job_details.length >= 50 ? '...' : '')"></td>
                    </template>
                    <template v-else>
                      <td>N/A</td>
                    </template>
                    <td>
                      <router-link :to="{ path: '/dashboard-jobapplications', query: { jobId: item.id } }" class="btn btn-sm btn-success">
                        <i class="fa-solid fa-person-circle-check"></i> <b>{{ item.job_applications }}</b>
                      </router-link>
                    </td>
                    <td>
                      <span v-if="item.is_approved == 'Yes'" class="badge bg-success"><b>Yes</b></span>
                      <span v-else class="badge bg-danger">No</span>
                    </td>
                    <td>
                      <span v-if="item.current_date <= item.application_deadline" class="badge bg-success"><b>Activated</b></span>
                    ...
                    <td class="">
                      <ul class="list-unstyled hstack gap-1 mb-0">                                       
                        ...
                        <li data-bs-toggle="tooltip" data-bs-placement="top" title="View">
                          <router-link :to="{ path: '/dashboard-viewjob', query: { jobId: item.id } }" class="btn btn-sm btn-primary">
                            <i class="fa-solid fa-eye"></i>
                          </router-link>
                        </li>

                        <li v-if="item.is_approved =='Yes'"  data-bs-toggle="tooltip" data-bs-placement="top" title="UnApprove">
                        ...
                        <!-- <li data-bs-toggle="tooltip" data-bs-placement="top" title="Applyed">
                            <router-link :to="{ path: '/dashboard-jobapplications', query: { jobId: item.id } }" class="btn btn-sm btn-success">
                              <i class="fa-solid fa-person-circle-check"></i> {{ item.job_applications }}
                            </router-link>
                        </li> -->
                      </ul>
                    </td>
          ...
          <div v-if="jobposts.length" class="d-flex">
            <div class="col-md-4 pl-0" style="margin: auto" v-if="jobposts.length">
    ...
    methods: {
      ...
      approvedStatus(id) {
        this.$swal
          ...
          .then((result) => {
            if (result.isConfirmed) {
              axios.get(`${this.backendUrl}job-post/approval/` + id)
              .then((response) => {
