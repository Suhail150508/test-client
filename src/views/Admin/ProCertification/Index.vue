        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List Of Professional Certifications</h4>
                        <span>
                            This is an Professional Certifications related information
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i> Add Professional Certifications</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tbody>
                        <tr v-for="(item, index) of proCertifications">
                            <td>{{ index+1 }}</td>
                            <td>{{ item.title }}</td>
                        ...
                        <tr v-show="!proCertifications.length">
                            <td colspan="7">
                    ...
                    <div v-if="proCertifications.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="proCertifications.length">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">ProCertification</h5>
                            ...
                            <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input v-model="form.organization" type="text" name="organization" class="form-control" placeholder="Organization" :readonly="showMode==true ? true : false">
                                        <HasError :form="form" field="organization" />
                                    </div>

                                    <div class="form-group">
                                        <label>Duration</label>
                                        <input v-model="form.duration" type="text" name="duration" class="form-control" placeholder="Duration" :readonly="showMode==true ? true : false">
...
<script>
    ...
    export default {
        name: 'CCCProCertificationsTable',
        ...
        methods: {
            getData(url) {
                this.$Progress.start();
                let linkUrl = url ? url :  `${this.backendUrl}professional-certification`;
                axios.get(linkUrl, {
                    params: {
                    ...
                    .then((response) => {
                        ...
                        this.proCertifications = response.data.data;
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}professional-certification`)
                    .then(response => {
                        this.getData()
                        console.log(response)
                        ...
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "ProCertifications Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}professional-certification/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "ProCertifications Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
