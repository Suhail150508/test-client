    ...
    <div class="col-12">
      <div class="card">
        <div class="row card-header bg-transparent border-bottom">
          <div class="col-md-8">
            <h4 class="card-title">List Of MajorMinor</h4>
            <span>
              This is an MajorMinor related information about Alumni and Student
            </span>
          </div>
          <div class="col-md-4">
            <button
            ...
            >
              <i class="fas fa-plus"></i>Add MajorMinor
            </button>
          </div>
        </div>
        ...
        <div class="card-body">
          ...
          <table class="table table-bordered dt-responsive nowrap w-100">
            ...
            <tbody>
              <tr v-for="(majorminor, index) of majorminors">
                <td>{{ majorminor.id }}</td>
                <td>{{ majorminor.title }}</td>
                <td>{{ majorminor.description }}</td>
                <td>Published</td>
                <td>
                  <button
                    type="button"
                    @click="show(majorminor)"
          ...
          <div v-if="majorminors.length" class="d-flex">
            <div
              class="col-md-4 pl-0"
              style="margin: auto"
              v-if="majorminors.length"
            >
...
export default {
  ...
  methods: {
    getData(url) {
      this.$Progress.start();
      let linkUrl = url ? url : `${this.backendUrl}major-minor`;
      axios
        .get(linkUrl, {
          params: {
        ...
        .then((response) => {
          this.majorminors = response.data.data;
    ...
    store() {
      ...
      this.form.busy = true;
      this.form
        .post(`${this.backendUrl}major-minor`)
        .then((response) => {
          this.getData();
    ...
    update() {
      ...
      this.form.busy = true;
      this.form
        .put(`${this.backendUrl}major-minor/` + this.form.id)
        .then((response) => {
          this.getData();
          $("#exampleModal").modal("hide");
          if (this.form.successful) {
        ...
        destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}major-minor/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
