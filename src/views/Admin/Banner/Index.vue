      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <table class="table table-bordered dt-responsive nowrap w-100">
            ...
            <tbody>
              <tr v-for="(banner, index) of banners">
                <td>{{ banner.id }}</td>
                <td>{{ banner.name }}</td>
                <td>{{ banner.title }}</td>
          ...
          <div v-if="banners.length" class="d-flex">
            <div
              class="col-md-4 pl-0"
              style="margin: auto"
              v-if="banners.length"
...
export default {
  ...
  methods: {
    getData(url) {
      this.$Progress.start();
      let linkUrl = url ? url : `${this.backendUrl}banners`;
      axios
        .get(linkUrl, {
          params: {
    ...
    update() {
      ...
      this.form.busy = true;
      this.form
        .put(`${this.backendUrl}banners/` + this.form.id)
        .then((response) => {
          this.getData();
          $("#exampleModal").modal("hide");
          if (this.form.successful) {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios.delete(`${this.backendUrl}banners/` + id).then((response) => {
