        ...
        <div class="col-12">
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        <h4 class="card-title">List Of Job Categories</h4>
                        <span>
                            This is an Job Categories related information
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i> Add Job Categories</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tbody>
                        <tr v-for="(item, index) of jobcategories">
                            <td>{{ index+1 }}</td>
                            <td>{{ item.title }}</td>
                            <td>{{ item.icon }}</td>
                    ...
                    <div v-if="jobcategories.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="jobcategories.length">
                            ...
                            <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)">
                                ...
                                <div class="modal-body row">
                                    <div class="col-xl-8">
                                        ...
                                        <div class="form-group">
                                            <label>Type</label>
                                            <select v-if="showMode==false" name="type" id="" class="form-control" v-model="form.type">
                                                <option value="">Select One</option>
                                                <option value="Functional">Functional</option>
                                                <option value="Special Skilled">Special Skilled</option>
                                            </select>
                                            <input v-if="showMode==true" v-model="form.type" type="text" class="form-control" placeholder="Type" :readonly="showMode==true ? true : false">
                                         ...
                                         <div class="form-group">
                                            ...
                                            <input v-if="showMode==true" v-model="form.is_approved" type="text" class="form-control" placeholder="Status" :readonly="showMode==true ? true : false">
                                    ...
                                    <div class="col-xl-4">
                                        <div class="card">
                                            ...
                                            <div class="card-body collapse" id="collapseCategories">
                                                <ul class="list-unstyled" v-for="(item, index) in jobcategories">
                                                    <li>
    ...
    export default {
        ...
        methods: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}job-category`)
                    .then(response => {
                        this.getData();
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "jobcategories Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}job-category/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "jobcategories Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}job-category/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
