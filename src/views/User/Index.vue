            ...
            <div class="card">
                <div class="row card-header bg-transparent border-bottom">
                    <div class="col-md-8">
                        ...
                        <span>
                            This is a User related information
                        </span>
                    </div>
                    <div class="col-md-4">
                        <button type="button" @click="create"  class="btn btn-sm btn-success float-end" > <i class="fas fa-plus"></i>Add User</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tr v-for="(user, index) of users">
                            ...
                            <td>
                                ...
                                <button type="button" @click="destroy(user.id)" class="btn btn-danger btn-sm">
                    ...
                    <div v-if="users.length" class="d-flex">
                        <div class="col-md-4 pl-0" style="margin: auto;" v-if="users.length">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group my-2">
                                        <label>Username</label>
                                            ...
                                            placeholder="Enter username" :readonly="showMode==true ? true : false"/>
                                            <HasError :form="form" field="username" />
                                    </div>

                                    <div class="form-group my-2">
                                        <label>Email</label>
                                        <input v-model="form.email" type="text" name="email" class="form-control" placeholder="Email" :readonly="showMode==true ? true : false">
                                        <HasError :form="form" field="email" />
                                    </div>

                                    <div class="form-group my-2">
                                        <label>Role</label>
                                        <select v-if="showMode==false" name="role_id" v-model="form.role_id" class="form-control">
                                            <option value="">Select One</option>
                                            <option v-for="role in roles" :value="role.id" v-html="role.name"></option>
                                        </select>
                                        <input v-if="showMode==true" v-model="form.role_id" type="text" name="role_id" class="form-control" placeholder="status" readonly>
...
<script>
    ...
    export default {
        ...
        methods: {
            getData(url) {
                this.$Progress.start();
                let linkUrl = url ? url :  `${this.backendUrl}user`;
                axios.get(linkUrl, {
                    params: {
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post(`${this.backendUrl}user`)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "User Added" });
                        }else{
                            this.$Progress.fail()
            ...
            update() {
                this.$Progress.start()
                this.form.busy = true
                this.form.put(`${this.backendUrl}user/`+this.form.id)
                    .then(response => {
                        this.getData()
                        $('#exampleModal').modal('hide')
                        if(this.form.successful){
                            this.$Progress.finish()
                            this.$notify({ type: "success", title: "Success", text: "User Updated" });
                        }else{
                            this.$Progress.fail()
                            this.$notify({type: "error", title: "Error", text: 'Something went wrong try again later'})
                        }
            ...
            destroy(id){
                ...
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`${this.backendUrl}user/`+id, )
                        .then(response => {
                            this.$swal.fire(
                                'Deleted!',
