              ...
              <div class="col-sm-4">
                <div class="row">
                  ...
                  <div class="col-sm-5">
                    <div>
                      <img
                        :src="$store.state.image" :alt="$store.state.alt"
      ...
      <div class="d-flex">
        ...
        <div class="dropdown d-inline-block">
          <button
            ...
            aria-expanded="false"
          >
            <img  class="rounded-circle header-profile-user" :src="$store.state.admin_avatar" :alt="$store.state.alt" />
          ...
          <div class="dropdown-menu dropdown-menu-end">
            ...
            <button class="dropdown-item text-danger">
              ...
              ></i>
              <span key="t-logout" @click="logout">Logout</span></button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods:{
    ...
    getAuthDetails(){
      if (Cookies.get('authIdCookie')){
        axios.get(`${this.backendUrl}get/user/`+Cookies.get('authIdCookie'))
          .then((response) => {
    ...
    logout() {
      if (Cookies.get('adminLoggedInTokenCookie') && Cookies.get('authIdCookie')){
        Cookies.remove('adminLoggedInTokenCookie')
        Cookies.remove('authIdCookie')
        if(!(Cookies.remove('adminLoggedInTokenCookie'))){
    ...
    /*logout(){
      ...
      console.log(localStorage.getItem('adminLoggedInToken'))

      if (localStorage.getItem('loggedInToken')){
        console.log(localStorage.getItem('loggedInToken'))
        localStorage.removeItem('loggedInToken')
        if(!(localStorage.removeItem('loggedInToken'))){
          this.$router.push({name: 'login'})
        }else{
          this.$router.push({name: 'app.dashboard'})
        }
      } else if (localStorage.getItem('adminLoggedInToken')){
        console.log(localStorage.getItem('adminLoggedInToken'))
        localStorage.removeItem('adminLoggedInToken')
        if(!(localStorage.removeItem('adminLoggedInToken'))){
          this.$router.push({name: 'AdminLogin'})
        }else{
